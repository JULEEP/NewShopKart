    ...
    if (firstName && lastName && email && password && password_confirmation, phone) {
      if (password === password_confirmation) {
        try {
          ...
          res.status(201).json({ status: true, message: "You registered successfully", "token": token })
        } catch (error) {
          console.log(error)
...
export { userRegistration, userLogin, updateUserPassword, loggeduser, otpSend, signOut }